<template>
<div>
  <b-navbar :sticky="true" variant="primary" type="dark">
    <b-navbar-nav>
      <b-nav-item @click="$router.push('/employees')">
				<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="white" class="bi bi-house" viewBox="0 0 16 16">
				<path fill-rule="evenodd" d="M2 13.5V7h1v6.5a.5.5 0 0 0 .5.5h9a.5.5 0 0 0 .5-.5V7h1v6.5a1.5 1.5 0 0 1-1.5 1.5h-9A1.5 1.5 0 0 1 2 13.5zm11-11V6l-2-2V2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5z"/>
				<path fill-rule="evenodd" d="M7.293 1.5a1 1 0 0 1 1.414 0l6.647 6.646a.5.5 0 0 1-.708.708L8 2.207 1.354 8.854a.5.5 0 1 1-.708-.708L7.293 1.5z"/>
				</svg>
			</b-nav-item>
      <b-nav-item v-if="isBreadCrumbNeed" href="#">
				<svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-chevron-right" viewBox="0 0 16 16">
					<path fill-rule="evenodd" d="M4.646 1.646a.5.5 0 0 1 .708 0l6 6a.5.5 0 0 1 0 .708l-6 6a.5.5 0 0 1-.708-.708L10.293 8 4.646 2.354a.5.5 0 0 1 0-.708z"/>
				</svg>
				<span class="font-weight-bold">{{breadCrumbName}}</span>
			</b-nav-item>
    </b-navbar-nav>
  </b-navbar>
</div>
</template>
<script>
export default {
	name:'Navbar',
	data() {
		return {
			isBreadCrumbNeed: false,
			breadCrumbName: ''
		}
	},
	watch: {
		$route() {
			this.setBreadCrumbText();
		}
	},
	mounted() {
		this.setBreadCrumbText();
	},
	methods: {
		setBreadCrumbText() {
			if(this.$route.path.includes('add')) {
				this.isBreadCrumbNeed = true;
				this.breadCrumbName = 'ADD';
			} else if(this.$route.path.includes('edit')) {
				this.isBreadCrumbNeed = true;
				this.breadCrumbName = 'EDIT';
			} else {
				this.isBreadCrumbNeed = false;
			}
		}
	}
}
</script>
<style scoped>
</style>